# RECEIPT — UI material generator (build) · 2026-08-03T04:52:37.418783+00:00
...
- **FAILED** `create UV (TextureCoordinate) UNVERIFIED` — **FAILED** Function "add_expression", input param "material_or_function" is required by the function input schema Json, but is missing from the incoming function input params Json.
Function schema Jso

## Findings
- **high** — create UV (TextureCoordinate) UNVERIFIED: **FAILED** Function "add_expression", input param "material_or_function" is required by the function input schema Json, but is missing from the incoming function input params Json.
Function schema Jso
