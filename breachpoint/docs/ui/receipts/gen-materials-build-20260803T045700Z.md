# RECEIPT — UI material generator (build) · 2026-08-03T04:57:00.924619+00:00

**Command:** `Tools/gen_ui/build_materials.py`
**Plan:** `Tools/gen_ui/material_plan.py` sha256 `370fe8aa80644be2`
...
domain MD_UI · blend BLEND_Translucent · 33 nodes · Angular ring mask, clockwise from 12. Sweep 0 = nothing, 1 = closed circle.
...
- PASS `wire P -> Px.Input (UNVERIFIED)` — {"returnValue":null}
- PASS `wire P -> Py.Input (UNVERIFIED)` — {"returnValue":null}
...
- PASS `wire Turns -> T.Input (UNVERIFIED)` — {"returnValue":null}
...
- PASS `wire AngRamp -> AngMask.Input (UNVERIFIED)` — {"returnValue":null}
...
- PASS `wire OuterRamp -> OuterMask.Input (UNVERIFIED)` — {"returnValue":null}
...
- PASS `wire InnerRamp -> InnerMask.Input (UNVERIFIED)` — {"returnValue":null}
...
- PASS `wire Color -> Alpha.B (UNVERIFIED)` — {"returnValue":null}
- PASS `output EmissiveColor <- Color (UNVERIFIED)` — {"returnValue":null}
- PASS `output Opacity <- Alpha (UNVERIFIED)` — {"returnValue":null}
...
## Verdict
PASS — material rebuilt from the plan, domain/blend verified by read-back, every parameter name confirmed present on the saved asset.
