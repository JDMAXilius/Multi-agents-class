...
8. **Zero Blueprint classes (R18).** Every actor, component, ability, effect,
   controller, and cue handler is C++. An engine asset exists only where UE 5.8
   offers no C++ authoring path, and that complete list — with who authors each —
   is **`BREACHPOINT-AUTHORING-MATRIX.md`**. GameplayEffects and GameplayCue
   handlers are C++ classes, not assets. Blockouts, table reimports, and input
   assets are generated by committed scripts, never hand-placed.
