...
struct FBNGrappleRoute
{
	...
	/** Short level name this route belongs to ("BR_Arena01"); empty = every map. Filtered
	 *  at world begin-play — a global table handed Arena01 points to bots on Spillway,
	 *  where they sat inside geometry (aib22 verify). */
	UPROPERTY(Config)
	FName Map;
...
public:
	...
	/** Keeps only this map's routes and snaps their points onto the navmesh (an authored
	 *  z 0 sits under the floor's nav at z 10); routes that do not project are dropped. */
...
private:
	...
	/** Generated by Tools/blockout/gen_grapple_routes.py — see FBNGrappleRoute. Config
	 *  is the full table; OnWorldBeginPlay reduces it to this map's projected routes. */
	UPROPERTY(Config)
