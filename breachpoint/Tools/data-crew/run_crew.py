...
def find_agents_dir() -> Path:
    """Locate the real crew definitions from wherever this script has been dropped.

    Three layouts, checked in order: the self-contained zip (`./agents`), the
    planning repo (`../../crew/.claude/agents`), and the game repo — where the
    crew lives at the repo root's `.claude/agents/` and this script sits some
    levels down in `Tools/data-crew/`. The last case is a walk rather than a
    fixed depth so relocating the script does not silently break it.
    """
    candidates = [
        HERE / "agents",                                      # zip layout
        HERE.parent.parent / "crew" / ".claude" / "agents",   # planning repo
    ]
    for parent in HERE.parents:                               # game repo
        candidates.append(parent / ".claude" / "agents")
    for cand in candidates:
        if (cand / "critic.md").exists():
            return cand
    sys.exit(
        "error: agent definitions not found — looked in ./agents, "
        "../../crew/.claude/agents, and .claude/agents in every parent directory"
    )
