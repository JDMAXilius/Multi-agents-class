...
def per_bot_summary(counts):
    ...
    for hit in counts["island_latched"]:
        row(hit["bot"])["island_latch_count"] += 1
    for hit in counts["egress_failed"]:
    ...
    for hit in counts["flank_over"]:         # Phase 15
        bot = row(hit["bot"])
        bot["flank_count"] += 1
...
AIBot: Alpha t=75.5 island latched — 3 draws with no full path inside 2500uu at (10,20,30)
...
AIBot: decide bot=3 seq=120 want=AIBot.Ambition.Engage s=0.812 over=AIBot.Ambition.Search rs=0.400 tac=AIBot.Tactic.Flank ts=0.700 commit=12 rng=1 facts=0a1b2c3d
AIBot: Alpha tactic -> AIBot.Tactic.Flank (0.70) over AIBot.Tactic.Push (0.50) reason=merit
AIBot: Alpha t=23.0 flank starts — point (40,50,30) detour 900uu
AIBot: Alpha t=26.5 flank over — arrived after 3.5s
AIBot: Bravo t=27.0 hold over — 2.5s at station
