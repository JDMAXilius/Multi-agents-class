...
UPlayMontageAndWaitForEvent* UPlayMontageAndWaitForEvent::PlayMontageAndWaitForEvent(UGameplayAbility* OwningAbility,
	FName TaskInstanceName, UAnimMontage* MontageToPlay, FGameplayTagContainer EventTags, float Rate, FName StartSection, bool bStopWhenAbilityEnds, float AnimRootMotionTranslationScale, float StartTimeSeconds)
{
	...
	//MyObj->CosmeticEvents = CosmeticEvents;
	MyObj->Rate = Rate;
	MyObj->StartSection = StartSection;
	MyObj->AnimRootMotionTranslationScale = AnimRootMotionTranslationScale;
	MyObj->bStopWhenAbilityEnds = bStopWhenAbilityEnds;
	MyObj->StartTimeSeconds = StartTimeSeconds;
	//MyObj->bSimulatedTask = !CosmeticEvents.IsEmpty();
...
void UPlayMontageAndWaitForEvent::Activate()
{
	...
	if (UAbilitySystemComponent* ASC = GetTargetASC())
	{
		...
		if (AnimInstance != nullptr)
		{
			...
			EventHandle = ASC->AddGameplayEventTagContainerDelegate(EventTags, FGameplayEventTagMulticastDelegate::FDelegate::CreateUObject(this, &UPlayMontageAndWaitForEvent::OnGameplayEvent));

			// if (!CosmeticEvents.IsEmpty())
			// {
			// 	if (UZoransAbilitySystemComponent* ZASC = Cast<UZoransAbilitySystemComponent>(ASC))
			// 	{
			// 		if (ZASC->PlayMontageWithCosmeticData(Ability, Ability->GetCurrentActivationInfo(), MontageToPlay, CosmeticEvents, Rate, StartSection, 0.f) > 0.f)
			// 		{
			// 			bPlayedMontage = true;
			// 		}
			// 	}
